...
static int des(const void *a, const void *b) {
  return (*(int*)b - *(int*)a);
}
...
static void create_runs(int *arr, int size, int run_size) {
  ...
  for (int i = 0; i < num_runs; i++) {
    qsort(arr + (i * run_size), run_size, sizeof(int), (int (*)(const void*, const void*))asc);
  }
}

// Swaps elements within a run with a certain probability
static void shuffle_within_runs(int *arr, int size, int run_size, double shuffle_prob) {
  for (int i = 0; i < size; i += run_size) {
    for (int j = 0; j < run_size; j++) {
      double ran = rand();
      int swap_index = rand() % run_size;
      if (ran / (double)RAND_MAX < shuffle_prob && i+j < size && i+swap_index < size) {
...
void create_sample(struct list_head *head, element_t *space, int samples, uint8_t type)
{
    srand(time(NULL));
    int run_size = 64;
